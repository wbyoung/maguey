 ...
 * For instance:
 *
 *     select('users').where({ firstName: 'Whitney' })
 *     // -> select * from users where firstName = ?, 'Whitney'
 *     select('users').where({ firstName: l('"Whitney"') })
 *     // -> select * from users where firstName = "Whitney"
 *
...
var LiteralString = Class.extend(/** @lends LiteralString# */ {
  ...
  /**
   * Converts to a native string. This is provided to ensure that the string
   * will appear naturally when used via many output options.
   *
