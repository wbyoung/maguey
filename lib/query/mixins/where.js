...
module.exports = Mixin.create(/** @lends Where# */ {
  ...
  /**
   * Filter results based on conditions. This method relies heavily on the use
   * of conditions. Using `Condition`, you can build complex queries. For
   * simple queries, simply pass in an object.
   *
   *     select('users').where({ id: 1 })
   *     select('users').where([{ first: 'Whitney' },
   *       w.or, { first: 'Whit' }], w.and, { last: 'Young' })
