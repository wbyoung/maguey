 ...
 *
 *     var first = w({ firstName: 'Whintey' }, w.or, { firstName: 'Whit' })
 *     w(first, w.and, { lastName: 'Young' })
 *
...
Condition.reopen(/** @lends Condition# */ {
  ...
  _validateUnaryOperations: function() {
    this._parts.forEach(function(part, index) {
      ...
      if (previous && !(previous instanceof Operation.__class__)) {
        ...
        throw new Error('Unary operator "' + part.name +
  ...
  _validateBinaryOperations: function() {
    this._parts.forEach(function(part, index) {
      ...
      var next = this._parts[index + 1];
      if (!previous) {
        throw new Error('Binary operator "' + part.name +
          '" must include left hand expression');
      }
      else if (!next) {
        throw new Error('Binary operator "' + part.name +
          '" must include right hand expression');
      }
      else if (previous instanceof Operation.Binary.__class__) {
        throw new Error('Binary operator "' + part.name +
          '" invalid after previous binary operator "' + previous.name + '"');
      }
      else if (previous instanceof Operation.Unary.__class__) {
        throw new Error('Binary operator "' + part.name +
          '" cannot follow unary "' + previous.name + '"');
      }
    }, this);
  },
   ...
   * `[w.not, w.not, Node.create()]` into a single node tree that represents
   ...
   * operations into a single tree node. For instance, it will convert
   * `[Node.create(), w.and, Node.create()]` into a single node tree that
...
Condition.reopen(/** @lends Condition# */ {
  ...
  transformExpressions: function(fn) {
    var transform = function(node) { // jscs:ignore jsDoc
  ...
  reduceFields: function(fn, accumulator) {
    var reduction = function(node) { // jscs:ignore jsDoc
...
Condition.reopenClass(/** @lends Condition */ {
   ...
   * @example
   *
   *     _extractPredicate('name[like]')
   *     // => { key: 'name', predicate: 'like' }
   *     _extractPredicate('id')
   *     // => { key: 'id', predicate: 'exact' }
   */
